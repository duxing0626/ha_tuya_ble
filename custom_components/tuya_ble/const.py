...
# Tuya IoT cloud login constants.
# Previously these were imported from `homeassistant.components.tuya.const`,
# but the core Tuya integration was rewritten (HA 2024.2) to use a QR-code /
# device-sharing login and no longer exposes them. This integration uses the
# classic Tuya IoT developer login, so the constants are bundled here to stay
# decoupled from the core integration.
